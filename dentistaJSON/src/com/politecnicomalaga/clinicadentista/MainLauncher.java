...
public class MainLauncher {
	...
	private static void saveClinica(Scanner sc) {
		System.out.println("Introduzca el fichero para grabar la copia de seguridad");
		String filename = leerStringTeclado(sc);

		if (miClinica!=null) {
			ControladorFicheros.writeText(filename, miClinica.toString(), true);

			Gson gson = new Gson();
			String dataJson = gson.toJson(miClinica);
			ControladorFicheros.writeText(filename+".json",dataJson, true);
		}
	}

	private static void loadClinica(Scanner sc) {
		System.out.println("Introduzca el fichero para cargar la copia de seguridad");
		System.out.println("Atención, los datos que haya introducido y no esten grabados\nse borrarán");
		System.out.println("Si no desea continuar, pulse Intro para salir");
		String filename = leerStringTeclado(sc);
		if (!filename.isBlank()){
			String json = ControladorFicheros.readText(filename);
			Gson gson = new Gson();
			miClinica = gson.fromJson(json, Clinica.class);
		}
	}
